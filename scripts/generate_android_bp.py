...
def write_blueprint_key_value(output, name, value, indent=1):
    ...
    if isinstance(value, list):
        output.append(f'{tabs(indent)}{name}: [')
        for item in value:
            output.append(f'{tabs(indent + 1)}"{item}",')
    ...
    if isinstance(value, bool):
        output.append(f"{tabs(indent)}{name}: {'true' if value else 'false'},")
        return
    output.append(f'{tabs(indent)}{name}: "{value}",')
...
def gn_deps_to_blueprint_deps(abi, target, build_info):
    ...
    if target in CODEGEN_TARGETS:
        ...
        defaults.append(f'{target_name}_android_codegen')
...
def main():
    ...
    infos = {}
    for abi in ABI_TARGETS:
        with open(args[f'gn_json_{gn_abi(abi)}'], 'r') as f:
    ...
    blueprint_targets = [
        ...
        (
            'cc_defaults',
            {
                'name': 'angle_common_library_cflags',
                'cflags': [
