...
def get_latest_success_build_info(bot_name):
    ...
    LOGGER.debug(f"Ran [bb ls '{bot_name}' -n 1 -status success -p]")
    out, err = bb.communicate()
    if err:
        raise ValueError(f"Unexpected error from bb ls: '{err}'")
    if not out:
        raise ValueError(f"Unexpected empty result from bb ls of bot '{bot_name}'")
    # Example output (line 1):
    # ci.chromium.org/b/8915280275579996928 SUCCESS   'chromium/ci/Win10 FYI dEQP Release (NVIDIA)/26877'
    # ...
    if 'SUCCESS' not in out:
        raise ValueError(f"Unexpected result from bb ls: '{out}'")
    info = {}
    ...
    if 'build_name' not in info:
        raise ValueError(f"Could not find build_name from bot '{bot_name}'")
    return info
...
def get_step_names(build_name):
    bb = subprocess.Popen(['bb', 'get', build_name, '-steps'],
                          stdout=subprocess.PIPE,
                          stderr=subprocess.PIPE)
    LOGGER.debug(f"Ran [bb get '{build_name}' -steps]")
    out, err = bb.communicate()
    if err:
        raise ValueError(f"Unexpected error from bb get: '{err}'")
    return [
        line.split('"')[1]
        for line in out.splitlines()
...
def validate_step_info(step_info, build_name, step_name):
    print_name = f"'{build_name}': '{step_name}'"
    if not step_info:
...
def get_step_info(build_name, step_name):
    bb = subprocess.Popen(['bb', 'log', build_name, step_name],
                          stdout=subprocess.PIPE,
                          stderr=subprocess.PIPE)
    LOGGER.debug(f"Ran [bb log '{build_name}' '{step_name}']")
    out, err = bb.communicate()
    if err:
        LOGGER.warning(
            f"Unexpected error from bb log '{build_name}' '{step_name}': '{err}'"
        )
        return None
    step_info = {}
    ...
    for line in out.splitlines():
        ...
        if workaround_prev_line_count > 0:
            ...
            workaround_prev_line = ""
            workaround_prev_line_count = 0
            LOGGER.debug(f"Formatting error workaround rebuilt line as: '{line}" + "'\n")
        ...
        if len(line_columns) is not 3:
            LOGGER.warning(f"Line improperly formatted: '{line}" + "'\n")
            continue
        ...
        if isInt:
            ...
            if len(step_info[key]) + len(append_string) < 50000:
                ...
                LOGGER.warning(f"Too many characters in column '{key}'. Output capped.")
        else:
...
def get_bot_info(bot_name):
    ...
    for step_name in info['step_names']:
        LOGGER.info(f"Parsing step '{step_name}'...")
...
def get_spreadsheet(service, spreadsheet_id):
    LOGGER.debug(f"Called [spreadsheets.get(spreadsheetId='{spreadsheet_id}')]")
    request = service.get(spreadsheetId=spreadsheet_id)
    if spreadsheet := request.execute():
        return spreadsheet
    else:
        raise Exception(f"Did not open spreadsheet '{spreadsheet_id}'")
...
def batch_update(service, spreadsheet_id, updates):
    ...
    LOGGER.debug(
        f"Called [spreadsheets.batchUpdate(spreadsheetId='{spreadsheet_id}', body={batch_update_request_body})]"
    )
...
def get_headers(service, spreadsheet_id, sheet_names):
    ...
    LOGGER.debug(
        f"Called [spreadsheets.values().batchGet(spreadsheetId='{spreadsheet_id}, ranges={header_ranges}')]"
    )
...
def batch_update_values(service, spreadsheet_id, data):
    ...
    LOGGER.debug(
        f"Called [spreadsheets.values().batchUpdate(spreadsheetId='{spreadsheet_id}', body={batch_update_values_request_body})]"
    )
    ...
    for bot_name in info:
        for step_name in info[bot_name]['step_names']:
            if step_name not in info[bot_name]:
                LOGGER.error(f"Missing info for step name: '{step_name}'")
            sheet_name = format_sheet_name(bot_name, step_name)
            headers_stale = False
...
def append_values(service, spreadsheet_id, sheet_name, values):
    ...
    LOGGER.debug(
        f"Called [spreadsheets.values().append(spreadsheetId='{spreadsheet_id}', body={append_values_request_body}, range='{header_range}', insertDataOption='{insert_data_option}', valueInputOption='{value_input_option}')]"
    )
...
def generate_duplicate_formula(headers, filter_columns):
    ...
    for i in range(len(headers)):
        if headers[i] == filter_columns[0]:
            col = str(i + 1)
            return f"IF(INDIRECT(ADDRESS(ROW(), {col}))=INDIRECT(ADDRESS(ROW() - 1, {col})),{generate_duplicate_formula(headers, filter_columns[1:])},0)"
        ...
        for step_name in info[bot_name]['step_names']:
            ...
            for key in headers[sheet_name]:
                if key in info[bot_name] and key in REQUIRED_COLUMNS:
                    ...
                    values.append('')
            LOGGER.info(f"Appending new rows to sheet '{sheet_name}'...")
            try:
...
def get_sheets_service(auth_path):
    ...
    if not os.path.exists(auth_path):
        LOGGER.info(f"Creating auth dir '{auth_path}'")
...
def main():
    ...
    for bot_name in BOT_NAMES:
        LOGGER.info(f"Parsing bot '{bot_name}'...")
        try:
    ...
    LOGGER.info(
        f'Info was successfully parsed to sheet: https://docs.google.com/spreadsheets/d/{args.spreadsheet}'
    )
